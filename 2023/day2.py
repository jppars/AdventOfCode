...
def part1():
    ...
    with open('day2_input.txt', 'r') as f:
        input = f.read()
    input = input + 'Dictionary:red=<=12:green=<=13:blue=<=14:'

    input = re.sub(r'\n', r'|', input)
    input = re.sub(r'Game (.)', r'\1', input)
    input = re.sub(r' (blue|red|green)(?=.*:\1=(.*?):)', r'\2', input)
    input = re.sub(r'\, ', r' and ', input)
    ...
    for line in input.split('|'):
        ...
        sum = sum + eval(results[0]) if eval(results[1]) else sum
...
def part2():
    ...
    with open('day2_input.txt', 'r') as f:
        for line in f:
            ...
            reds = re.sub(r'(?:^|;)(?!.*?red.*?(?:;|$)).*?(?:;|$)', r'', line)
            reds = re.sub(r'(?:^|;).*?([0-9]*) red.*?(?=;|$)', r'\1,', reds)
            min_reds = eval('max([{}])'.format(reds[:-1]))

            greens = re.sub(r'(?:^|;)(?!.*?green.*?(?:;|$)).*?(?:;|$)', r'', line)
            greens = re.sub(r'(?:^|;).*?([0-9]*) green.*?(?=;|$)', r'\1,', greens)
            min_greens = eval('max([{}])'.format(greens[:-1]))

            blues = re.sub(r'(?:^|;)(?!.*?blue.*?(?:;|$)).*?(?:;|$)', r'', line)
            blues = re.sub(r'(?:^|;).*?([0-9]*) blue.*?(?=;|$)', r'\1,', blues)
            min_blues = eval('max([{}])'.format(blues[:-1]))
